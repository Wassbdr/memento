...
function NodeDetailPanel({
  ...
  return (
    <motion.aside
      ...
      className="absolute right-0 top-0 h-full w-72 z-30 glass border-l border-white/10 p-5 overflow-y-auto flex flex-col"
    >
      {/* Header */}
      <div className="flex items-start justify-between mb-5">
        ...
        <button
          onClick={onClose}
          className="p-1.5 rounded-lg hover:bg-white/10 text-white/35 hover:text-white/70 transition-colors flex-shrink-0"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="space-y-3.5 flex-1">
        ...
        {node.details?.emotion && (
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="text-rose-300">{node.details.emotion}</span>
          </div>
        )}
        ...
        {node.details?.significance !== undefined && (
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="text-white/40">Importance émotionnelle</span>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {node.details.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 text-white/35 border border-white/10"
                >
                  #{tag}
                </span>
              ))}
...
const LEGEND = [
  { color: "#f59e0b", label: "Patiente" },
  { color: "#a78bfa", label: "Personnes" },
  { color: "#86efac", label: "Aidants" },
  { color: "#2dd4bf", label: "Lieux" },
  { color: "#60a5fa", label: "Routines" },
  { color: "#f472b6", label: "Épisodes" },
];
...
export default function MemoryGraph() {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden" ref={containerRef}>
      ...
      >
        <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-teal-400/60 mb-2">
          Graphe de mémoire
        </p>
      ...
      <motion.div
        ...
        className="absolute bottom-6 left-6 z-10 glass rounded-xl p-3 space-y-1.5"
      >
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        transition={{ delay: 1 }}
        className="absolute bottom-6 right-6 z-10 text-[10px] text-white/20 pointer-events-none"
      >
      ...
      <div className="force-graph-container w-full h-full">
        <ForceGraph2D
          ...
          linkColor={() => "rgba(124, 111, 205, 0.18)"}
          linkWidth={1}
          linkDirectionalParticles={2}
          linkDirectionalParticleWidth={1.5}
          linkDirectionalParticleColor={() => "rgba(124, 111, 205, 0.45)"}
