...
function AlertCard({ alert, index }: { alert: Alert; index: number }) {
  ...
  return (
    <motion.article
      ...
      exit="exit"
      layout
      className={`glass rounded-2xl p-4 cursor-pointer select-none border ${cfg.border} transition-colors duration-200 hover:bg-white/[0.04]`}
...
export default function AlertFeed() {
  ...
  return (
    <div className="h-full flex flex-col px-6 py-8 max-w-2xl mx-auto w-full">
      ...
      >
        <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-violet-400/60 mb-2">
          Fil de suivi
        </p>
        <h1 className="text-2xl font-display font-bold text-white leading-tight">
          Préoccupations &amp; Soins
        </h1>
        <p className="text-sm text-white/35 mt-1">Mamie Rose · Aujourd'hui</p>
      </motion.header>
